...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        if selective_scan_fn is None:
            raise ImportError(
                "VMUNet requires mamba-ssm. Install optional dependencies with "
                "`pip install -r requirements-mamba.txt`."
            ) from _selective_scan_import_error
        self.selective_scan = selective_scan_fn
...
class VSSM(nn.Module):
    ...
    def forward(self, x):
        x1, skip_list = self.forward_features(x)
        x2 = self.forward_features_up(x1, skip_list)
        x3 = self.forward_final(x2)
        return x3
